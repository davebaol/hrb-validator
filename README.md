...
:pushpin: Keep in mind that all leaf validators, without exception, have their shortcut opt (not reported in the list below).

Leaf Validator                         | Type at `path` | Description
:--------------------------------------|:--------------:|:--------------------------------------
...
**isDivisibleBy(path, number)**        |number<br/>string| Check if the value at `path` is a number or a string representing a number that's divisible by the specified number.
...
**isHexadecimal(path)** :raised_hand:            |string| Check if the string is a hexadecimal number.
...
**isIn(path, values)** :exclamation:   |string| Check if the string is in a array of allowed values. Removed in favour of isOneOf.
...
:pushpin: Keep in mind that only branch validators taking a `path` as first argument have a shortcut opt (not reported in the list below).

Branch Validator                | Type at `path` | Description
:-------------------------------|:--------------:|:------------------------------------
